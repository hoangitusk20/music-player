...
const app = {
  ...
  handleEvents: function () {
    ...
    var darkmode = playerOptionElement.querySelector(
      ".player__option-dark-mode"
    );
    var filterFavoriteElement =
      playerOptionElement.querySelector(".filter-favorite");
    ...
    progress.addEventListener("input", function () {
      _this.audioElement.currentTime =
        (progress.value / 100) * _this.audioElement.duration;
    });
    ...
    itemRemove.forEach((item) => {
      item.addEventListener("click", function () {
        ...
        var index = Array.from(_this.playlistElement.children).indexOf(
          playItemElement
        );
    ...
    playerOptionElement.addEventListener("click", function () {
      let playerOptionMenu = playerOptionElement.querySelector(
        ".player__option-menu"
      );
    ...
    _this.playlistElement.addEventListener("click", function (e) {
      ...
      if (optionElement) {
        optionElement
          .querySelector(".play-item__option-menu")
          .classList.toggle("active");
        return;
      }
      var songNode = e.target.closest(".play-item:not(.active)");
      if (songNode) {
        _this.currentIndex = Array.from(_this.playlistElement.children).indexOf(
          songNode
        );
        changeSong();
      }
  ...
  loadSong: function () {
    ...
    if (
      curentSongElement.offsetTop + curentSongElement.clientHeight >
        window.innerHeight + window.scrollY ||
      curentSongElement.offsetTop - player.clientHeight < window.scrollY
    ) {
      curentSongElement.scrollIntoView({
        behavior: "smooth",
        block: "center",
        inline: "nearest",
      });
